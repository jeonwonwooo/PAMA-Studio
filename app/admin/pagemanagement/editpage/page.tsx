...
export default function EditPage() {
  ...
  const [form, setForm] = useState({ title: pageTitle, description: pageDesc });
  const [image, setImage] = useState<string | null>(null);
  const [published, setPublished] = useState(true);
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    setForm({ title: pageTitle, description: pageDesc });
  }, [pageTitle, pageDesc]);
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <h1 className="text-2xl font-bold text-gray-800 mt-1">Edit Page Details</h1>
        </div>
        <button
        ...
        <div>
          ...
          {image ? (
            ...
            <label className="flex flex-col items-center justify-center w-full h-40 bg-[#FBF7F1] rounded-xl border-2 border-dashed border-[#8B1A1A]/20 cursor-pointer hover:bg-[#8B1A1A]/5 transition">
              ...
              <span className="text-sm text-gray-400">Klik untuk ganti gambar</span>
