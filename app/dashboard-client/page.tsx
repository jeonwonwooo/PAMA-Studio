  ...
  return (
    ...
    >
      <motion.div
        ...
        className="relative w-full max-w-lg overflow-hidden rounded-[32px] bg-[#FBF7F1] shadow-2xl"
      >
        <button onClick={onClose} className="absolute right-6 top-6 text-[#3a1a1a]/40 hover:text-[#8B1A1A] transition-colors">
          <X size={24} />
        </button>
        <div className="bg-[#8B1A1A] p-8 text-white">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xl font-bold" style={{ fontFamily: "Fraunces, serif" }}>Detail Pesanan</h2>
        ...
        <div className="p-8 space-y-6" style={{ fontFamily: "Inter Tight, sans-serif" }}>
          ...
          <div className="space-y-3">
            <button
              ...
              className="flex w-full items-center justify-center gap-3 rounded-full bg-[#25D366] py-4 text-sm font-bold text-white shadow-lg transition hover:bg-[#1da851] active:scale-[0.98]"
            >
...
export default function ClientDashboard() {
  ...
  const userName = profile?.full_name?.split(" ")[0] || "Versi Terbaikmu";
      ...
      <div className="max-w-6xl mx-auto px-5 pt-10 space-y-10">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            ...
            <h1 className="text-4xl md:text-5xl font-serif text-[#1a0505] leading-tight" style={{ fontFamily: "Fraunces, serif" }}>
              Halo, <span className="italic font-light text-[#8B1A1A]">{userName}!</span>
            </h1>
          ...
          <Link
            href="/paket"
            className="inline-flex items-center gap-2 px-6 py-3 bg-[#8B1A1A] text-white text-sm font-semibold rounded-full hover:bg-[#6B1212] transition-all"
          >
            Pesan Paket Baru <ChevronRight size={16} />
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            ...
            { label: "Selesai", value: orders.filter(o => o.status === 'done').length, color: "bg-green-600" }
          ].map((stat, i) => (
            <motion.div
              key={i}
              ...
              className="group relative overflow-hidden rounded-[32px] bg-white border border-[#8B1A1A]/10 p-8 shadow-sm transition-all hover:shadow-md"
            >
        ...
        <div className="space-y-6">
          ...
          {loading ? (
            <div className="grid gap-4">
              {[1, 2, 3].map(i => <div key={i} className="h-24 bg-white/50 rounded-[32px] border border-[#8B1A1A]/5 animate-pulse" />)}
            </div>
          ) : orders.length === 0 ? (
            <div className="bg-white/50 rounded-[32px] border border-dashed border-[#8B1A1A]/20 p-16 text-center">
              ...
              <p className="text-[#3a1a1a]/40 text-sm mb-4">Ayo abadikan momen terbaikmu sekarang!</p>
